...
# {user_id: {"state": "awaiting_insight_selection" | "awaiting_report_selection", "etfs": [...]}}
_user_states: dict = {}
        ...
        try:
            ...
            if 0 <= idx < len(etfs):
                ...
                if state["state"] == "awaiting_insight_selection":
                    ...
                    await _send_etf_report(message.channel, etfs[idx])
            else:
...
async def _handle_report(message: discord.Message):
    ...
    await message.channel.send("\n".join(lines))
    _user_states[message.author.id] = {
        "state": "awaiting_report_selection",
        "etfs": etfs,
    }


async def _send_etf_report(channel: discord.DMChannel, etf: dict):
    row = db.get_latest_daily_report(etf["id"])
    if not row:
        await channel.send(f"**{etf['name']}**의 리포트 데이터가 아직 없습니다.\n다음 정기 리포트(08:00 KST) 이후에 다시 시도해주세요.")
        return
    ...
    await _send_chunked(channel, header + row["report_text"])
