...
다음 형식을 정확히 따라 한국어로 답변하세요.
**가독성 규칙: 각 종목/변화는 반드시 별도 줄에 작성하고, 섹션 사이에 빈 줄을 넣으세요.**

**오늘의 주요 변화**

- 신규 편입 종목은 각각 한 줄씩: `▲ 종목명 X.X% 편입 — 한 줄 배경`
- 청산 종목은 각각 한 줄씩: `▼ 종목명 청산 — 한 줄 배경`
- 의도적 매수/매도는 각각 한 줄씩: `🔼 종목명 X.X%→X.X% (+Nshares) — 한 줄 배경`
- 변화가 없으면: `변화 없음 — 전 포지션 유지`

**운용 의도 분석**

이 변화들을 종합한 운용역의 의도를 서술하세요.
**문장마다 줄바꿈하여 작성하세요. 한 줄에 한 문장만.**
매크로 환경, 테마 사이클, 리스크 관리 관점을 고려하세요.

**주목할 점**

투자자 관점에서 가장 중요한 1가지를 1-2문장으로 명확히 제시하세요.

## 분석 품질 기준

- 종목마다 반드시 별도 줄 — 여러 종목을 한 문장에 나열하지 말 것
- 비중(%), 수량 변화량을 수치로 포함
- 운용 의도는 근거 있는 추측: "~일 가능성이 높다", "~를 시사한다" 형식 사용
- 과거 인사이트가 있으면 오늘 변화가 해당 패턴의 연장인지 새로운 시그널인지 판단
- 베이스라인 비율이 1.0에서 크게 벗어난 날에는 설정/해지 규모 영향을 언급"""
